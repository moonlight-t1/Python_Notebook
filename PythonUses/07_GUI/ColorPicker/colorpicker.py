...
def captureWindow(x=0, y=0, width=0, height=0):
    hwin = win32gui.GetDesktopWindow()  # 최상위 윈도우의 핸들값 구한다
    hwindc = win32gui.GetWindowDC(hwin)  # 최상위 윈도우에 대한 DC 구조체 구한다(DC의 핸들)
    # DC를 생성하는데 핸들값(hwindc)으로 생성한다, 현재 화면에 대한 정보의 DC
    srcdc = win32ui.CreateDCFromHandle(hwindc)
    memdc = srcdc.CreateCompatibleDC()  # srcdc의 복사본 생성
    bmp = win32ui.CreateBitmap()  # 비트맵 객체 생성
    bmp.CreateCompatibleBitmap(srcdc, width, height)  # srcdc 로 비트맵 작성
    memdc.SelectObject(bmp)  # memedc에 비트맵 장착
    memdc.BitBlt((0, 0), (width, height), srcdc,
                 (x, y), win32con.SRCCOPY)  # 비트맵 복사
    sigIntArray = bmp.GetBitmapBits(True)  # 실제 픽셀 추출
    img = np.fromstring(sigIntArray, dtype="uint8")  # opencv 형태로 데이터 가공
    img.shape = (height, width, 4)  # RGBA 4채널
...
class MyPicker(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.worker = Worker()
        # sig_updata와 updateWindow를 connect
    ...
    def updateWindow(self, pt):
        ...
        if self.zoom > 1:
            print(self.zoom)
            zoom_image = self.resize(
                capture, width=self.zoom * capture.shape[1])
            if zoom_image is not None:
                center = int(zoom_image.shape[0] /
                             2), int(zoom_image.shape[1] / 2)
                # 이미지 크롭 [y1:y2, x1:x2]
                capture = zoom_image[center[0] - center_y:center[0] +
                                     center_y, center[1] - center_x:center[1] + center_x]

        capture = cv2.cvtColor(capture, cv2.COLOR_RGB2BGR)
        bgr = capture[center_y, center_x]  # 한 픽셀 캡쳐
        self.currentBGR = (bgr[0], bgr[1], bgr[2])

        hsv = colorsys.rgb_to_hsv(bgr[2]/255, bgr[1]/255, bgr[0]/255)
        hsv_angle = (int(hsv[0] * 360), int(hsv[1] * 100),
                     int(hsv[2] * 100))  # 포토샵 스타일 hsv

        bgr_inverse = self.get_inverse_color(bgr)  # 보색

        out_code, text_color, bg_color = self.get_code(bgr)
        out_inverse, text_color_inverse, bg_color_inverse = self.get_code(
            bgr_inverse)

        out_hsv = "{}˚ {}% {}%".format(
            hsv_angle[0], hsv_angle[1], hsv_angle[2])
        ...
        if self.saveBGR[0][0] >= 0:
            out_save1, text_color_save1, bg_color_save1 = self.get_code(
                self.saveBGR[0])
        if self.saveBGR[1][0] >= 0:
            out_save2, text_color_save2, bg_color_save2 = self.get_code(
                self.saveBGR[1])

        if self.saveBGR[0][0] >= 0 and self.saveBGR[1][0] >= 0:
            nR = self.saveBGR[0][2] + \
                (int((self.saveBGR[1][2] - self.saveBGR[0][2]) / 2) * 1)
            nG = self.saveBGR[0][1] + \
                (int((self.saveBGR[1][1] - self.saveBGR[0][1]) / 2) * 1)
            nB = self.saveBGR[0][0] + \
                (int((self.saveBGR[1][0] - self.saveBGR[0][0]) / 2) * 1)
        ...
        # 십자가
        step = int(width / 10)
        cv2.line(capture, (center_x - step, center_y),
                 (center_x + step, center_y), (255, 255, 255), 1)
        cv2.line(capture, (center_x, center_y - step),
                 (center_x, center_y + step), (255, 255, 255), 1)

        # PyQt에서 출력 가능한 Q 이미지로 변환
        qImage = QtGui.QImage(capture.data, width, height,
                              3 * width, QtGui.QImage.Format_RGB888)
